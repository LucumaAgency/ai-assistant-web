...
router.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    const recentAttempts = await User.getRecentLoginAttempts(email, ip_address, 15);
    if (recentAttempts >= 5) {
    ...
    // Crear sesión
    const sessionToken = await Token.createSession({
      user_id: user.id!,
      session_token: '',
      ip_address,
...
router.post('/logout', async (req: Request, res: Response) => {
  try {
    ...
    if (sessionToken) {
      await Token.deleteSession(sessionToken);
    }
...
router.post('/reset-password', async (req: Request, res: Response) => {
  try {
    ...
    // Verificar token
    const resetToken = await Token.verifyPasswordResetToken(token);
    ...
    // Cambiar contraseña
    const success = await User.changePassword(resetToken.user_id, newPassword);
    ...
    await Token.markPasswordTokenAsUsed(token);

    // Revocar todos los refresh tokens del usuario
    await Token.revokeAllUserRefreshTokens(resetToken.user_id);
...
router.post('/refresh', async (req: Request, res: Response) => {
  try {
    ...
    await Token.revokeRefreshToken(refreshToken);
    const newRefreshToken = await Token.createRefreshToken(user.id!);
